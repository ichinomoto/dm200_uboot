...
#if 1
#define RSHIFT_GPIO 12
#define ALT_GPIO    13
#define CTRL_GPIO   14
#define LSHIFT_GPIO 15
#define POWER_GPIO  2
#define KEY_ON  0
#define KEY_OFF 1
...
#ifdef CONFIG_WARP
bool check_recovery_boot (void)
#else
static bool check_recovery_boot(void)
#endif
{
	if(gpio_get_value(RSHIFT_GPIO) == KEY_ON &&  gpio_get_value(ALT_GPIO) == KEY_ON
		&& gpio_get_value(POWER_GPIO) == KEY_ON){
		printf("***** RECOVORY BOOT!! *****\n");
		return true;
	}
	printf("***** NORMAL BOOT!! *****\n");
	return false;
...
static rk_boot_img_hdr * rk_load_image_from_storage(const disk_partition_t* ptn, bootm_headers_t *pimage)
{
	...
	if (memcmp(hdr->magic, BOOT_MAGIC,
		...
		if(recovery_flag){
			if (rkimage_load_image(hdr, ptn, get_disk_partition("recover_kernel")) != 0) {
...
int do_bootrk(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	recovery_flag = check_recovery_boot();
	...
	if(recovery_flag){
		if((ptn = get_disk_partition("recover_boot")) == NULL){
